...
- Alerts are edge-triggered with a 15-second per-message repeat limit. Speech is non-preemptive: the current sentence always finishes before another begins. Critical notices may move ahead of routine notices that are still waiting, but never interrupt the active sentence. The waiting queue is bounded to four items; stale (over eight seconds), resolved and environment-mismatched waiting notices are discarded. Scope changes, preference changes, muting and an explicit stop may cancel speech.\n\nAfter the two-second freshness limit, attitude instruments retain the last confirmed sample instead of dimming or predicting motion. The console labels the hold state and continues showing data age. This is a display hold only: the current aircraft condition is unknown, graphs preserve the missing-data gap, and no browser extrapolation is presented as measured telemetry.
